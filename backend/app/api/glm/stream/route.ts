...
function getProvider(bodyProvider?: string): LlmProvider {
  ...
  if (raw === "openai_compatible" || raw === "openai-compatible" || raw === "openai") {
...
function getOpenAiCompatibleConfig() {
  ...
  try {
    headers = JSON.parse(process.env.OPENAI_COMPAT_HEADERS ?? "{}") as Record<string, string>;
  } catch {
    headers = {};
  }
...
export async function POST(req: Request) {
  ...
  if (provider === "openrouter" || provider === "openai_compatible") {
    ...
    const { apiKey, baseUrl, model, httpReferer, appTitle } = base as {
      apiKey: string;
      baseUrl: string;
      model: string;
      httpReferer: string;
      ...
      return Response.json(
        {
          error:
            provider === "openrouter"
              ...
              : "Missing OpenAI-compatible API key (set OPENAI_COMPAT_API_KEY)",
        },
        { status: 500 }
      );
    }
