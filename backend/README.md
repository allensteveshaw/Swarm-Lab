...
- `LLM_PROVIDER`：LLM 提供方（`glm` / `openrouter` / `openai_compatible`，默认 `glm`）
...
- `OPENAI_COMPAT_API_KEY`：OpenAI 兼容服务 API Key（当 `LLM_PROVIDER=openai_compatible` 时可用）
- `OPENAI_COMPAT_BASE_URL`（可选）：默认 `http://127.0.0.1:11434/v1/chat/completions`
- `OPENAI_COMPAT_MODEL`（可选）：OpenAI 兼容模型名
- `OPENAI_COMPAT_HEADERS`（可选）：JSON headers
- `AGENT_SHELL_MODE`（可选）：`auto|powershell|cmd|bash`，控制内置 `bash` 工具的 shell
